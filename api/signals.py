...
def update_equipament_on_device_save(sender, instance, **kwargs):
    try:
        ...
        if instance.horimeter is not None:
            equipament.horimetro_inicialSuntech = float(instance.horimeter)
            equipament.save()
            logger.info(f"Equipamento {equipament.id} atualizado com horímetro Suntech: {equipament.horimetro_inicialSuntech}")
        else:
